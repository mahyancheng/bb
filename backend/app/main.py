...
# main.py is in /app/app, so ROOT needs to go up one level to /app
...
if FRONTEND_DIR.is_dir() and (FRONTEND_DIR / "index.html").is_file():
    try:
        ...
        print(f"Successfully mounted static files from {FRONTEND_DIR} at '/'.")
         ...
         # Provide a fallback message if mounting fails
         @app.get("/")
    ...
    # Provide a fallback message if the directory isn't found
    @app.get("/")
